...
export default function BrowsePetsPage() {
  ...
  const fetchPets = async () => {
    ...
    try {
      ...
      console.log("[BrowsePetsPage] Fetched pets:", result);
      if (result && result.success) {
        console.log("[BrowsePetsPage] Pets loaded:", result.pets);
        result.pets?.forEach((pet, idx) => {
          console.log(`[BrowsePetsPage] Pet ${idx + 1}:`, {
            id: pet.id,
            name: pet.name,
      ...
      console.error("[BrowsePetsPage] Error:", err);
            ...
            {!loading && filteredPets.length > 0 && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredPets.map((pet) => {
                  console.log("[BrowsePetsPage] Rendering pet card:", { name: pet.name, hasImage: !!pet.image_url, imageUrl: pet.image_url });
                  return (
                  <div key={pet.id} className="group bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-md hover:translate-y-1 transition-transform" onClick={() => handlePetClick(pet.id)}>
                    {pet.image_url ? (
                      <img 
                        ...
                        onError={(e) => console.log("[BrowsePetsPage] Image failed to load:", pet.image_url, e)}
                        onLoad={() => console.log("[BrowsePetsPage] Image loaded successfully:", pet.image_url)}
                      />
                    ) : (
